...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    let xMagnitude = Math.sqrt(Math.pow(x1, 2) + Math.pow(y1, 2))
    let yMagnitude = Math.sqrt(Math.pow(x2, 2) + Math.pow(y2, 2))
...
function toNumber(value, def) {
    return isNaN(new Number(value)) ? def : new Number(value);
}
